...
function memoryUI() {
  memory.flies.forEach((fly,i)=>{const preferences=responses(memory,i),fans=preferences.map((v,j)=>v>=VOLUNTEER_THRESHOLD?j:-1).filter(j=>j>=0),button=$('flyCards').children[i];button.classList.toggle('selected',selected.has(i));button.setAttribute('aria-pressed',String(selected.has(i)));button.setAttribute('aria-label',`${fly.name}. ${fans.length?'Likes '+fans.map(j=>FRUITS[j].name).join(', '):'No learned favorites'}. ${selected.has(i)?'Selected for training':'Select for training'}`);button.innerHTML=`<span class="chef-name">${fly.name}</span><span class="chef-number">0${i+1}</span><span class="smell-icons">${fans.length?fans.map(j=>FRUITS[j].emoji).join(''):'· · ·'}</span><span class="chef-note">${fly.lessons} smell + snack pairings</span>`;});
...
function drawBrain() {
  ...
  text('brainTitle',`${fly.name}’s learned smell response`);
...
function updateUI() {
  ...
  $('brainCaption').textContent=motorView?'Fixed PN → KC · |activity| ×15 · six engineered outputs':'319 neurons · smell activity · schematic positions';
  $('brainCaption').title=motorView?'Flight inputs and six force/torque outputs are engineered. Glows show absolute PN/KC activity amplified 15×; the MBON is not a motor neuron.':'Glows show smell-circuit rate activity, not measured spikes. Positions are illustrative.';
  if(motorView){
