...
import {createFlightWorld,yawQuaternion,poseQuaternion,teacherMotorActions} from './flight3d.js';
...
export function motorObservation(w){
  ...
  const q=yawQuaternion(w.target.yaw).multiply(poseQuaternion(w).invert()),s=q.w<0?-1:1;
...
export function motorActions(swarm,world,mode='learned'){
  ...
  const {pn,kc}=activity(swarm,motorObservation(world),mode);
  return world.flies.map((fly,index)=>{
    const a=swarm.agents[index];a.lastPNActivity=fly.enabled?[...pn]:pn.map(()=>0);a.lastActivity=fly.enabled?[...kc]:kc.map(()=>0);
    const out=a.readout.map(row=>mode==='untrained'||!fly.enabled?0:row.reduce((s,w,j)=>s+w*kc[j],0));
