# Fruit Fly Fruit Ninja — original motor laboratory
...
Open http://127.0.0.1:5184/lab.html. The server listens only on loopback. Keep its terminal running; Ctrl+C stops it. If the port is already occupied, the launcher prints an error instead of stopping another project. Internet access is not required for the simulation.
...
- `src/app.js`, `lab.html`, `style.css`: interactive kitchen and inspection controls.
