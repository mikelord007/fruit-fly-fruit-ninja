...
Fruit Fly Fruit Ninja is a 3D salad game about learned preferences and learned knife control. It is not a claim that real flies cooperate with knives or that playing improves human neuroplasticity.
...
Validated flight readouts, including deliberate erasure, persist in browser storage independently from taste memory. Saved policies must match the circuit IDs, contact-count hash, encoding version, and finite weight dimensions. The selected mode defaults to autopilot on reload. Training can run while the game continues; the new lesson takes effect when fitting completes. Round resets and taste resets preserve the current flight mode and readout.

The learned motor policy controls only the attached knife and its rigidly coupled carriers. The coordinator still chooses targets and grip assignments, and perch approach/return paths remain authored 3D splines in every mode. A fly reaching the knife in Untrained mode is therefore expected; it cannot lift the knife with zero motor output.
...
**Flight circuit** view instead shows the selected carrier's actual PN and KC motor features, mapped by neuron ID. Magnitudes are amplified 15× for visibility; ACTIVE FEATURES counts raw KC magnitudes above 0.001. The six engineered motor outputs are not biological MBONs, so the MBON/APL/PPL1 nodes and their edges stay inactive in this view. MOTOR FORCE shows the bounded force-vector magnitude actually applied to that carrier, in normalized simulation units. Autopilot and off-duty carriers show an idle motor circuit. Untrained carriers can have active features but zero force. Switching flight mode chooses a useful initial Brain view, and the player can select either view at any time.
